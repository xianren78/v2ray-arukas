...
RUN apt-get update && apt-get -y install --no-install-recommends wget ca-certificates openssh-server openssh-sftp-server unzip bsdmainutils \
 && apt-get clean \
...
ADD files/traffic.sh /etc/v2ray/traffic.sh
RUN chmod +x /etc/v2ray/traffic.sh
